...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fklientedit = new ew.Form("fklientedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fklientedit;

    // Add fields
    var fields = currentTable.fields;
    fklientedit.addFields([
        ["klientID", [fields.klientID.visible && fields.klientID.required ? ew.Validators.required(fields.klientID.caption) : null], fields.klientID.isInvalid],
    ...
    // Form_CustomValidate
    fklientedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fklientedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fklientedit.lists.klientTipi = <?= $Page->klientTipi->toClientList($Page) ?>;
    fklientedit.lists.klientAutori = <?= $Page->klientAutori->toClientList($Page) ?>;
    loadjs.done("fklientedit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fklientedit" id="fklientedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->klientID->Visible) { // klientID ?>
    <div id="r_klientID"<?= $Page->klientID->rowAttributes() ?>>
        <label id="elh_klient_klientID" class="<?= $Page->LeftColumnClass ?>"><?= $Page->klientID->caption() ?><?= $Page->klientID->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->klientID->cellAttributes() ?>>
<span id="el_klient_klientID">
<span<?= $Page->klientID->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->klientID->getDisplayValue($Page->klientID->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="klient" data-field="x_klientID" data-hidden="1" name="x_klientID" id="x_klientID" value="<?= HtmlEncode($Page->klientID->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
