...
// Page object
$MakinaImportiEdit = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { makina_importi: currentTable } });
var currentForm, currentPageID;
var fmakina_importiedit;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fmakina_importiedit = new ew.Form("fmakina_importiedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fmakina_importiedit;

    // Add fields
    var fields = currentTable.fields;
    fmakina_importiedit.addFields([
        ["mimpID", [fields.mimpID.visible && fields.mimpID.required ? ew.Validators.required(fields.mimpID.caption) : null], fields.mimpID.isInvalid],
    ...
    // Form_CustomValidate
    fmakina_importiedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fmakina_importiedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fmakina_importiedit.lists.mimpMarka = <?= $Page->mimpMarka->toClientList($Page) ?>;
    fmakina_importiedit.lists.mimpModeli = <?= $Page->mimpModeli->toClientList($Page) ?>;
    fmakina_importiedit.lists.mimpTipi = <?= $Page->mimpTipi->toClientList($Page) ?>;
    fmakina_importiedit.lists.mimpKarburant = <?= $Page->mimpKarburant->toClientList($Page) ?>;
    fmakina_importiedit.lists.mimpKambio = <?= $Page->mimpKambio->toClientList($Page) ?>;
    fmakina_importiedit.lists.mimpGati = <?= $Page->mimpGati->toClientList($Page) ?>;
    loadjs.done("fmakina_importiedit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fmakina_importiedit" id="fmakina_importiedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->mimpID->Visible) { // mimpID ?>
    <div id="r_mimpID"<?= $Page->mimpID->rowAttributes() ?>>
        <label id="elh_makina_importi_mimpID" class="<?= $Page->LeftColumnClass ?>"><?= $Page->mimpID->caption() ?><?= $Page->mimpID->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->mimpID->cellAttributes() ?>>
<span id="el_makina_importi_mimpID">
<span<?= $Page->mimpID->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->mimpID->getDisplayValue($Page->mimpID->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="makina_importi" data-field="x_mimpID" data-hidden="1" name="x_mimpID" id="x_mimpID" value="<?= HtmlEncode($Page->mimpID->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<?php $Page->mimpMarka->EditAttrs->prepend("onchange", "ew.updateOptions.call(this);"); ?>
    <select
        ...
        data-select2-id="fmakina_importiedit_x_mimpMarka"
...
<script>
loadjs.ready("fmakina_importiedit", function() {
    var options = { name: "x_mimpMarka", selectId: "fmakina_importiedit_x_mimpMarka" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fmakina_importiedit.lists.mimpMarka.lookupOptions.length) {
        options.data = { id: "x_mimpMarka", form: "fmakina_importiedit" };
    } else {
        options.ajax = { id: "x_mimpMarka", form: "fmakina_importiedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<?php $Page->mimpModeli->EditAttrs->prepend("onchange", "ew.updateOptions.call(this);"); ?>
    <select
        ...
        data-select2-id="fmakina_importiedit_x_mimpModeli"
        data-table="makina_importi"
...
<script>
loadjs.ready("fmakina_importiedit", function() {
    var options = { name: "x_mimpModeli", selectId: "fmakina_importiedit_x_mimpModeli" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fmakina_importiedit.lists.mimpModeli.lookupOptions.length) {
        options.data = { id: "x_mimpModeli", form: "fmakina_importiedit" };
    } else {
        options.ajax = { id: "x_mimpModeli", form: "fmakina_importiedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_makina_importi_mimpTipi">
    <select
        ...
        data-select2-id="fmakina_importiedit_x_mimpTipi"
        data-table="makina_importi"
...
<script>
loadjs.ready("fmakina_importiedit", function() {
    var options = { name: "x_mimpTipi", selectId: "fmakina_importiedit_x_mimpTipi" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fmakina_importiedit.lists.mimpTipi.lookupOptions.length) {
        options.data = { id: "x_mimpTipi", form: "fmakina_importiedit" };
    } else {
        options.ajax = { id: "x_mimpTipi", form: "fmakina_importiedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<script>
loadjs.ready(["fmakina_importiedit", "editor"], function() {
    ew.createEditor("fmakina_importiedit", "x_mimpInfo", 35, 4, <?= $Page->mimpInfo->ReadOnly || false ? "true" : "false" ?>);
});
</script>
</span>
</div></div>
...
<script>
loadjs.ready(["fmakina_importiedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fmakina_importiedit", "x_mimpDtHyrjes", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
<?php
    if (in_array("makina_importi_sherbime", explode(",", $Page->getCurrentDetailTable())) && $makina_importi_sherbime->DetailEdit) {
?>
