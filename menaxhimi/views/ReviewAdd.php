...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    freviewadd = new ew.Form("freviewadd", "add");
    currentPageID = ew.PAGE_ID = "add";
    currentForm = freviewadd;

    // Add fields
    var fields = currentTable.fields;
    freviewadd.addFields([
    ...
    // Form_CustomValidate
    freviewadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    freviewadd.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    freviewadd.lists.reviewGjuha = <?= $Page->reviewGjuha->toClientList($Page) ?>;
    freviewadd.lists.reviewPer = <?= $Page->reviewPer->toClientList($Page) ?>;
    freviewadd.lists.reviewAktiv = <?= $Page->reviewAktiv->toClientList($Page) ?>;
    loadjs.done("freviewadd");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="freviewadd" id="freviewadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<input type="hidden" name="fa_x_reviewFoto" id= "fa_x_reviewFoto" value="0">
...
<script>
loadjs.ready(["freviewadd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("freviewadd", "x_reviewDate", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
