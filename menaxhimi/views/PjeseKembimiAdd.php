...
// Page object
$PjeseKembimiAdd = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { pjese_kembimi: currentTable } });
var currentForm, currentPageID;
var fpjese_kembimiadd;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fpjese_kembimiadd = new ew.Form("fpjese_kembimiadd", "add");
    currentPageID = ew.PAGE_ID = "add";
    currentForm = fpjese_kembimiadd;

    // Add fields
    var fields = currentTable.fields;
    fpjese_kembimiadd.addFields([
        ...
        ["pjeseShtuar", [fields.pjeseShtuar.visible && fields.pjeseShtuar.required ? ew.Validators.required(fields.pjeseShtuar.caption) : null], fields.pjeseShtuar.isInvalid]
    ]);

    // Form_CustomValidate
    fpjese_kembimiadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fpjese_kembimiadd.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fpjese_kembimiadd.lists.pjeseGjendja = <?= $Page->pjeseGjendja->toClientList($Page) ?>;
    fpjese_kembimiadd.lists.pjeseAutori = <?= $Page->pjeseAutori->toClientList($Page) ?>;
    loadjs.done("fpjese_kembimiadd");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fpjese_kembimiadd" id="fpjese_kembimiadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
