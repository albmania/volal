...
// Page object
$ServisPjesetAdd = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { servis_pjeset: currentTable } });
var currentForm, currentPageID;
var fservis_pjesetadd;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fservis_pjesetadd = new ew.Form("fservis_pjesetadd", "add");
    currentPageID = ew.PAGE_ID = "add";
    currentForm = fservis_pjesetadd;

    // Add fields
    var fields = currentTable.fields;
    fservis_pjesetadd.addFields([
    ...
    // Form_CustomValidate
    fservis_pjesetadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fservis_pjesetadd.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fservis_pjesetadd.lists.servisPjesePjesa = <?= $Page->servisPjesePjesa->toClientList($Page) ?>;
    loadjs.done("fservis_pjesetadd");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fservis_pjesetadd" id="fservis_pjesetadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<span id="el_servis_pjeset_servisPjesePjesa">
    <select
        ...
        data-select2-id="fservis_pjesetadd_x_servisPjesePjesa"
...
<script>
loadjs.ready("fservis_pjesetadd", function() {
    var options = { name: "x_servisPjesePjesa", selectId: "fservis_pjesetadd_x_servisPjesePjesa" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fservis_pjesetadd.lists.servisPjesePjesa.lookupOptions.length) {
        options.data = { id: "x_servisPjesePjesa", form: "fservis_pjesetadd" };
    } else {
        options.ajax = { id: "x_servisPjesePjesa", form: "fservis_pjesetadd", limit: 20 };
    }
