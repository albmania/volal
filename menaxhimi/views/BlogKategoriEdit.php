...
// Page object
$BlogKategoriEdit = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { blog_kategori: currentTable } });
var currentForm, currentPageID;
var fblog_kategoriedit;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fblog_kategoriedit = new ew.Form("fblog_kategoriedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fblog_kategoriedit;

    // Add fields
    var fields = currentTable.fields;
    fblog_kategoriedit.addFields([
        ["blogKatID", [fields.blogKatID.visible && fields.blogKatID.required ? ew.Validators.required(fields.blogKatID.caption) : null], fields.blogKatID.isInvalid],
    ...
    // Form_CustomValidate
    fblog_kategoriedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fblog_kategoriedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fblog_kategoriedit.lists.blogKatGjuha = <?= $Page->blogKatGjuha->toClientList($Page) ?>;
    loadjs.done("fblog_kategoriedit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fblog_kategoriedit" id="fblog_kategoriedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->blogKatID->Visible) { // blogKatID ?>
    <div id="r_blogKatID"<?= $Page->blogKatID->rowAttributes() ?>>
        <label id="elh_blog_kategori_blogKatID" class="<?= $Page->LeftColumnClass ?>"><?= $Page->blogKatID->caption() ?><?= $Page->blogKatID->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->blogKatID->cellAttributes() ?>>
<span id="el_blog_kategori_blogKatID">
<span<?= $Page->blogKatID->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->blogKatID->getDisplayValue($Page->blogKatID->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="blog_kategori" data-field="x_blogKatID" data-hidden="1" name="x_blogKatID" id="x_blogKatID" value="<?= HtmlEncode($Page->blogKatID->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
