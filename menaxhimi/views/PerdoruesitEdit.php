...
// Page object
$PerdoruesitEdit = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { perdoruesit: currentTable } });
var currentForm, currentPageID;
var fperdoruesitedit;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fperdoruesitedit = new ew.Form("fperdoruesitedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fperdoruesitedit;

    // Add fields
    var fields = currentTable.fields;
    fperdoruesitedit.addFields([
        ["perdID", [fields.perdID.visible && fields.perdID.required ? ew.Validators.required(fields.perdID.caption) : null], fields.perdID.isInvalid],
    ...
    // Form_CustomValidate
    fperdoruesitedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fperdoruesitedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fperdoruesitedit.lists.perdNiveliPerdoruesit = <?= $Page->perdNiveliPerdoruesit->toClientList($Page) ?>;
    loadjs.done("fperdoruesitedit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fperdoruesitedit" id="fperdoruesitedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->perdID->Visible) { // perdID ?>
    <div id="r_perdID"<?= $Page->perdID->rowAttributes() ?>>
        <label id="elh_perdoruesit_perdID" class="<?= $Page->LeftColumnClass ?>"><?= $Page->perdID->caption() ?><?= $Page->perdID->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->perdID->cellAttributes() ?>>
<span id="el_perdoruesit_perdID">
<span<?= $Page->perdID->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->perdID->getDisplayValue($Page->perdID->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="perdoruesit" data-field="x_perdID" data-hidden="1" name="x_perdID" id="x_perdID" value="<?= HtmlEncode($Page->perdID->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<span id="el_perdoruesit_perdNiveliPerdoruesit">
    <select
        ...
        data-select2-id="fperdoruesitedit_x_perdNiveliPerdoruesit"
...
<script>
loadjs.ready("fperdoruesitedit", function() {
    var options = { name: "x_perdNiveliPerdoruesit", selectId: "fperdoruesitedit_x_perdNiveliPerdoruesit" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fperdoruesitedit.lists.perdNiveliPerdoruesit.lookupOptions.length) {
        options.data = { id: "x_perdNiveliPerdoruesit", form: "fperdoruesitedit" };
    } else {
        options.ajax = { id: "x_perdNiveliPerdoruesit", form: "fperdoruesitedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<script>
loadjs.ready(["fperdoruesitedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fperdoruesitedit", "x_perdDtReg", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
