...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fmakinaadd = new ew.Form("fmakinaadd", "add");
    currentPageID = ew.PAGE_ID = "add";
    currentForm = fmakinaadd;

    // Add fields
    var fields = currentTable.fields;
    fmakinaadd.addFields([
    ...
    // Form_CustomValidate
    fmakinaadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fmakinaadd.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fmakinaadd.lists.makinaKlienti = <?= $Page->makinaKlienti->toClientList($Page) ?>;
    fmakinaadd.lists.makinaKarburanti = <?= $Page->makinaKarburanti->toClientList($Page) ?>;
    fmakinaadd.lists.makinaShiturVOLAL = <?= $Page->makinaShiturVOLAL->toClientList($Page) ?>;
    loadjs.done("fmakinaadd");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fmakinaadd" id="fmakinaadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<div class="input-group flex-nowrap">
    <select
        ...
        data-select2-id="fmakinaadd_x_makinaKlienti"
        data-table="makina"
...
<script>
loadjs.ready("fmakinaadd", function() {
    var options = { name: "x_makinaKlienti", selectId: "fmakinaadd_x_makinaKlienti" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fmakinaadd.lists.makinaKlienti.lookupOptions.length) {
        options.data = { id: "x_makinaKlienti", form: "fmakinaadd" };
    } else {
        options.ajax = { id: "x_makinaKlienti", form: "fmakinaadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
