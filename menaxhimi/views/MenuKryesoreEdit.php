...
// Page object
$MenuKryesoreEdit = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { menu_kryesore: currentTable } });
var currentForm, currentPageID;
var fmenu_kryesoreedit;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fmenu_kryesoreedit = new ew.Form("fmenu_kryesoreedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fmenu_kryesoreedit;

    // Add fields
    var fields = currentTable.fields;
    fmenu_kryesoreedit.addFields([
        ["menukID", [fields.menukID.visible && fields.menukID.required ? ew.Validators.required(fields.menukID.caption) : null], fields.menukID.isInvalid],
    ...
    // Form_CustomValidate
    fmenu_kryesoreedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fmenu_kryesoreedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fmenu_kryesoreedit.lists.menukGjuha = <?= $Page->menukGjuha->toClientList($Page) ?>;
    fmenu_kryesoreedit.lists.menukBlank = <?= $Page->menukBlank->toClientList($Page) ?>;
    fmenu_kryesoreedit.lists.menukAktiv = <?= $Page->menukAktiv->toClientList($Page) ?>;
    fmenu_kryesoreedit.lists.menukAutor = <?= $Page->menukAutor->toClientList($Page) ?>;
    loadjs.done("fmenu_kryesoreedit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fmenu_kryesoreedit" id="fmenu_kryesoreedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->menukID->Visible) { // menukID ?>
    <div id="r_menukID"<?= $Page->menukID->rowAttributes() ?>>
        <label id="elh_menu_kryesore_menukID" class="<?= $Page->LeftColumnClass ?>"><?= $Page->menukID->caption() ?><?= $Page->menukID->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->menukID->cellAttributes() ?>>
<span id="el_menu_kryesore_menukID">
<span<?= $Page->menukID->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->menukID->getDisplayValue($Page->menukID->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="menu_kryesore" data-field="x_menukID" data-hidden="1" name="x_menukID" id="x_menukID" value="<?= HtmlEncode($Page->menukID->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
