...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fmakina_tipiadd = new ew.Form("fmakina_tipiadd", "add");
    currentPageID = ew.PAGE_ID = "add";
    currentForm = fmakina_tipiadd;

    // Add fields
    var fields = currentTable.fields;
    fmakina_tipiadd.addFields([
    ...
    // Form_CustomValidate
    fmakina_tipiadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fmakina_tipiadd.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fmakina_tipiadd.lists.mtipiMarka = <?= $Page->mtipiMarka->toClientList($Page) ?>;
    fmakina_tipiadd.lists.mtipiModeli = <?= $Page->mtipiModeli->toClientList($Page) ?>;
    loadjs.done("fmakina_tipiadd");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fmakina_tipiadd" id="fmakina_tipiadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php $Page->mtipiMarka->EditAttrs->prepend("onchange", "ew.updateOptions.call(this);"); ?>
    <select
        ...
        data-select2-id="fmakina_tipiadd_x_mtipiMarka"
...
<script>
loadjs.ready("fmakina_tipiadd", function() {
    var options = { name: "x_mtipiMarka", selectId: "fmakina_tipiadd_x_mtipiMarka" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fmakina_tipiadd.lists.mtipiMarka.lookupOptions.length) {
        options.data = { id: "x_mtipiMarka", form: "fmakina_tipiadd" };
    } else {
        options.ajax = { id: "x_mtipiMarka", form: "fmakina_tipiadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_makina_tipi_mtipiModeli">
    <select
        ...
        data-select2-id="fmakina_tipiadd_x_mtipiModeli"
        data-table="makina_tipi"
...
<script>
loadjs.ready("fmakina_tipiadd", function() {
    var options = { name: "x_mtipiModeli", selectId: "fmakina_tipiadd_x_mtipiModeli" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fmakina_tipiadd.lists.mtipiModeli.lookupOptions.length) {
        options.data = { id: "x_mtipiModeli", form: "fmakina_tipiadd" };
    } else {
        options.ajax = { id: "x_mtipiModeli", form: "fmakina_tipiadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
