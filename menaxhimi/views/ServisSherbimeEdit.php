...
// Page object
$ServisSherbimeEdit = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { servis_sherbime: currentTable } });
var currentForm, currentPageID;
var fservis_sherbimeedit;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fservis_sherbimeedit = new ew.Form("fservis_sherbimeedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fservis_sherbimeedit;

    // Add fields
    var fields = currentTable.fields;
    fservis_sherbimeedit.addFields([
    ...
    // Form_CustomValidate
    fservis_sherbimeedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fservis_sherbimeedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fservis_sherbimeedit.lists.servisSherbimSherbimi = <?= $Page->servisSherbimSherbimi->toClientList($Page) ?>;
    loadjs.done("fservis_sherbimeedit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fservis_sherbimeedit" id="fservis_sherbimeedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->servisSherbimID->Visible) { // servisSherbimID ?>
    <div id="r_servisSherbimID"<?= $Page->servisSherbimID->rowAttributes() ?>>
        <label id="elh_servis_sherbime_servisSherbimID" class="<?= $Page->LeftColumnClass ?>"><?= $Page->servisSherbimID->caption() ?><?= $Page->servisSherbimID->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->servisSherbimID->cellAttributes() ?>>
<span id="el_servis_sherbime_servisSherbimID">
<span<?= $Page->servisSherbimID->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->servisSherbimID->getDisplayValue($Page->servisSherbimID->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="servis_sherbime" data-field="x_servisSherbimID" data-hidden="1" name="x_servisSherbimID" id="x_servisSherbimID" value="<?= HtmlEncode($Page->servisSherbimID->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<span id="el_servis_sherbime_servisSherbimSherbimi">
    <select
        ...
        data-select2-id="fservis_sherbimeedit_x_servisSherbimSherbimi"
...
<script>
loadjs.ready("fservis_sherbimeedit", function() {
    var options = { name: "x_servisSherbimSherbimi", selectId: "fservis_sherbimeedit_x_servisSherbimSherbimi" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fservis_sherbimeedit.lists.servisSherbimSherbimi.lookupOptions.length) {
        options.data = { id: "x_servisSherbimSherbimi", form: "fservis_sherbimeedit" };
    } else {
        options.ajax = { id: "x_servisSherbimSherbimi", form: "fservis_sherbimeedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
