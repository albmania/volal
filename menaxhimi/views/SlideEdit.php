...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fslideedit = new ew.Form("fslideedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fslideedit;

    // Add fields
    var fields = currentTable.fields;
    fslideedit.addFields([
        ["slideID", [fields.slideID.visible && fields.slideID.required ? ew.Validators.required(fields.slideID.caption) : null], fields.slideID.isInvalid],
        ...
        ["slideAzhornuar", [fields.slideAzhornuar.visible && fields.slideAzhornuar.required ? ew.Validators.required(fields.slideAzhornuar.caption) : null], fields.slideAzhornuar.isInvalid]
    ]);
    ...
    fslideedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fslideedit.lists.slideGjuha = <?= $Page->slideGjuha->toClientList($Page) ?>;
    fslideedit.lists.slideTarget = <?= $Page->slideTarget->toClientList($Page) ?>;
    fslideedit.lists.slideAktiv = <?= $Page->slideAktiv->toClientList($Page) ?>;
    fslideedit.lists.slideAutori = <?= $Page->slideAutori->toClientList($Page) ?>;
...
$Page->showMessage();
?>
<form name="fslideedit" id="fslideedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->slideID->Visible) { // slideID ?>
    <div id="r_slideID"<?= $Page->slideID->rowAttributes() ?>>
        <label id="elh_slide_slideID" class="<?= $Page->LeftColumnClass ?>"><?= $Page->slideID->caption() ?><?= $Page->slideID->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->slideID->cellAttributes() ?>>
<span id="el_slide_slideID">
<span<?= $Page->slideID->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->slideID->getDisplayValue($Page->slideID->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="slide" data-field="x_slideID" data-hidden="1" name="x_slideID" id="x_slideID" value="<?= HtmlEncode($Page->slideID->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<input type="hidden" name="fa_x_slideFoto" id= "fa_x_slideFoto" value="<?= (Post("fa_x_slideFoto") == "0") ? "0" : "1" ?>">
