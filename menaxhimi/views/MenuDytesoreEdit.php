...
// Page object
$MenuDytesoreEdit = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { menu_dytesore: currentTable } });
var currentForm, currentPageID;
var fmenu_dytesoreedit;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fmenu_dytesoreedit = new ew.Form("fmenu_dytesoreedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fmenu_dytesoreedit;

    // Add fields
    var fields = currentTable.fields;
    fmenu_dytesoreedit.addFields([
        ["menudID", [fields.menudID.visible && fields.menudID.required ? ew.Validators.required(fields.menudID.caption) : null], fields.menudID.isInvalid],
        ...
        ["menudAzhornuar", [fields.menudAzhornuar.visible && fields.menudAzhornuar.required ? ew.Validators.required(fields.menudAzhornuar.caption) : null], fields.menudAzhornuar.isInvalid]
    ]);

    // Form_CustomValidate
    fmenu_dytesoreedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fmenu_dytesoreedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fmenu_dytesoreedit.lists.menudGjuha = <?= $Page->menudGjuha->toClientList($Page) ?>;
    fmenu_dytesoreedit.lists.menudKryesore = <?= $Page->menudKryesore->toClientList($Page) ?>;
    fmenu_dytesoreedit.lists.menudBlank = <?= $Page->menudBlank->toClientList($Page) ?>;
    fmenu_dytesoreedit.lists.menudAktiv = <?= $Page->menudAktiv->toClientList($Page) ?>;
    fmenu_dytesoreedit.lists.menudAutor = <?= $Page->menudAutor->toClientList($Page) ?>;
    loadjs.done("fmenu_dytesoreedit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fmenu_dytesoreedit" id="fmenu_dytesoreedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->menudID->Visible) { // menudID ?>
    <div id="r_menudID"<?= $Page->menudID->rowAttributes() ?>>
        <label id="elh_menu_dytesore_menudID" class="<?= $Page->LeftColumnClass ?>"><?= $Page->menudID->caption() ?><?= $Page->menudID->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->menudID->cellAttributes() ?>>
<span id="el_menu_dytesore_menudID">
<span<?= $Page->menudID->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->menudID->getDisplayValue($Page->menudID->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="menu_dytesore" data-field="x_menudID" data-hidden="1" name="x_menudID" id="x_menudID" value="<?= HtmlEncode($Page->menudID->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<div class="input-group flex-nowrap">
    <select
        ...
        data-select2-id="fmenu_dytesoreedit_x_menudKryesore"
        data-table="menu_dytesore"
...
<script>
loadjs.ready("fmenu_dytesoreedit", function() {
    var options = { name: "x_menudKryesore", selectId: "fmenu_dytesoreedit_x_menudKryesore" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fmenu_dytesoreedit.lists.menudKryesore.lookupOptions.length) {
        options.data = { id: "x_menudKryesore", form: "fmenu_dytesoreedit" };
    } else {
        options.ajax = { id: "x_menudKryesore", form: "fmenu_dytesoreedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
