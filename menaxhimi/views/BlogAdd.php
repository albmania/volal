...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fblogadd = new ew.Form("fblogadd", "add");
    currentPageID = ew.PAGE_ID = "add";
    currentForm = fblogadd;
    ...
    fblogadd.addFields([
        ...
        ["blogShtuar", [fields.blogShtuar.visible && fields.blogShtuar.required ? ew.Validators.required(fields.blogShtuar.caption) : null], fields.blogShtuar.isInvalid]
    ]);

    // Form_CustomValidate
    fblogadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fblogadd.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fblogadd.lists.blogGjuha = <?= $Page->blogGjuha->toClientList($Page) ?>;
    fblogadd.lists.blogKategoria = <?= $Page->blogKategoria->toClientList($Page) ?>;
    fblogadd.lists.blogAutori = <?= $Page->blogAutori->toClientList($Page) ?>;
...
$Page->showMessage();
?>
<form name="fblogadd" id="fblogadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<span id="el_blog_blogKategoria">
    <select
        ...
        data-select2-id="fblogadd_x_blogKategoria"
        data-table="blog"
...
<script>
loadjs.ready("fblogadd", function() {
    var options = { name: "x_blogKategoria", selectId: "fblogadd_x_blogKategoria" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fblogadd.lists.blogKategoria.lookupOptions.length) {
        options.data = { id: "x_blogKategoria", form: "fblogadd" };
    } else {
        options.ajax = { id: "x_blogKategoria", form: "fblogadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
loadjs.ready(["fblogadd", "editor"], function() {
    ew.createEditor("fblogadd", "x_blogTxt", 35, 4, <?= $Page->blogTxt->ReadOnly || false ? "true" : "false" ?>);
});
</script>
</span>
</div></div>
...
<input type="hidden" name="fa_x_blogFoto" id= "fa_x_blogFoto" value="0">
...
<script>
loadjs.ready(["fblogadd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fblogadd", "x_blogDtPublik", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
