...
class SlideEdit extends Slide
{
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("slideID") ?? Key(0) ?? Route(2)) !== null) {
                $this->slideID->setQueryStringValue($keyValue);
                $this->slideID->setOldValue($this->slideID->QueryStringValue);
            } elseif (Post("slideID") !== null) {
                $this->slideID->setFormValue(Post("slideID"));
                $this->slideID->setOldValue($this->slideID->FormValue);
            } else {
            ...
            if (Post("action") !== null) {
                ...
                if (($keyValue = Get("slideID") ?? Route("slideID")) !== null) {
                    ...
                    $this->slideID->CurrentValue = null;
                }
            }
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->slideID->IsDetailKey) {
            $this->slideID->setFormValue($val);
        }
        ...
        // Check field name 'slideAzhornuar' first before field var 'x_slideAzhornuar'
        $val = $CurrentForm->hasValue("slideAzhornuar") ? $CurrentForm->getValue("slideAzhornuar") : $CurrentForm->getValue("x_slideAzhornuar");
        if (!$this->slideAzhornuar->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->slideAzhornuar->Visible = false; // Disable update for API request
            } else {
                $this->slideAzhornuar->setFormValue($val);
            }
            $this->slideAzhornuar->CurrentValue = UnFormatDateTime($this->slideAzhornuar->CurrentValue, $this->slideAzhornuar->formatPattern());
        }
		$this->slideFoto->OldUploadPath = '../ngarkime/slide/';
		$this->slideFoto->UploadPath = $this->slideFoto->OldUploadPath;
    ...
    {
        global $CurrentForm;
        $this->slideID->CurrentValue = $this->slideID->FormValue;
        ...
        $this->slideAzhornuar->CurrentValue = $this->slideAzhornuar->FormValue;
        $this->slideAzhornuar->CurrentValue = UnFormatDateTime($this->slideAzhornuar->CurrentValue, $this->slideAzhornuar->formatPattern());
    }
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
        if ($this->slideID->Required) {
            if (!$this->slideID->IsDetailKey && EmptyValue($this->slideID->FormValue)) {
                $this->slideID->addErrorMessage(str_replace("%s", $this->slideID->caption(), $this->slideID->RequiredErrorMessage));
            }
        }
        ...
        if ($this->slideAzhornuar->Required) {
            if (!$this->slideAzhornuar->IsDetailKey && EmptyValue($this->slideAzhornuar->FormValue)) {
                $this->slideAzhornuar->addErrorMessage(str_replace("%s", $this->slideAzhornuar->caption(), $this->slideAzhornuar->RequiredErrorMessage));
            }
        }
    ...
    protected function editRow()
    {
        ...
        // slideGjuha
        $this->slideGjuha->setDbValueDef($rsnew, $this->slideGjuha->CurrentValue, "", $this->slideGjuha->ReadOnly);

        // slideFoto
        if ($this->slideFoto->Visible && !$this->slideFoto->ReadOnly && !$this->slideFoto->Upload->KeepFile) {
            $this->slideFoto->Upload->DbValue = $rsold['slideFoto']; // Get original value
        ...
        // slideTxt1
        $this->slideTxt1->setDbValueDef($rsnew, $this->slideTxt1->CurrentValue, null, $this->slideTxt1->ReadOnly);

        // slideTxt2
        $this->slideTxt2->setDbValueDef($rsnew, $this->slideTxt2->CurrentValue, "", $this->slideTxt2->ReadOnly);

        // slideTxt3
        $this->slideTxt3->setDbValueDef($rsnew, $this->slideTxt3->CurrentValue, null, $this->slideTxt3->ReadOnly);

        // slideButonTxt
        $this->slideButonTxt->setDbValueDef($rsnew, $this->slideButonTxt->CurrentValue, "", $this->slideButonTxt->ReadOnly);

        // slideLink
        $this->slideLink->setDbValueDef($rsnew, $this->slideLink->CurrentValue, "", $this->slideLink->ReadOnly);

        // slideTarget
        $this->slideTarget->setDbValueDef($rsnew, $this->slideTarget->CurrentValue, "", $this->slideTarget->ReadOnly);

        // slideRadha
        $this->slideRadha->setDbValueDef($rsnew, $this->slideRadha->CurrentValue, 0, $this->slideRadha->ReadOnly);

        // slideAktiv
        $this->slideAktiv->setDbValueDef($rsnew, $this->slideAktiv->CurrentValue, "", $this->slideAktiv->ReadOnly);
        ...
        // slideAzhornuar
        $this->slideAzhornuar->CurrentValue = CurrentDateTime();
        $this->slideAzhornuar->setDbValueDef($rsnew, $this->slideAzhornuar->CurrentValue, null);
        ...
        if ($this->slideFoto->Visible && !$this->slideFoto->Upload->KeepFile) {
            ...
            if (!EmptyValue($this->slideFoto->Upload->FileName)) {
                ...
                $this->slideFoto->setDbValueDef($rsnew, $this->slideFoto->Upload->FileName, "", $this->slideFoto->ReadOnly);
            }
        }
