    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("reviewID") ?? Route("reviewID")) !== null) {
                ...
                $this->RecKey["reviewID"] = $this->reviewID->QueryStringValue;
            } elseif (Post("reviewID") !== null) {
                $this->reviewID->setFormValue(Post("reviewID"));
                $this->RecKey["reviewID"] = $this->reviewID->FormValue;
            } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
                $this->reviewID->setQueryStringValue($keyValue);
                $this->RecKey["reviewID"] = $this->reviewID->QueryStringValue;
            } elseif (!$loadCurrentRecord) {
                $returnUrl = "ReviewList"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                        ...
                        if (!$res) { // Load record based on key
                            ...
                            $returnUrl = "ReviewList"; // No matching record, return to list
                        }
                    break;
            }
        } else {
            $returnUrl = "ReviewList"; // Not page request, return to list
        }
        if ($returnUrl != "") {
