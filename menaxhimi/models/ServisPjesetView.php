 ...
 * Page class
 */
class ServisPjesetView extends ServisPjeset
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ServisPjesetView";
    ...
    public function __construct()
    {
        ...
        if (($keyValue = Get("servisPjeseID") ?? Route("servisPjeseID")) !== null) {
            $this->RecKey["servisPjeseID"] = $keyValue;
        }
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("servisPjeseID") ?? Route("servisPjeseID")) !== null) {
                ...
                $this->RecKey["servisPjeseID"] = $this->servisPjeseID->QueryStringValue;
            } elseif (Post("servisPjeseID") !== null) {
                $this->servisPjeseID->setFormValue(Post("servisPjeseID"));
                $this->RecKey["servisPjeseID"] = $this->servisPjeseID->FormValue;
            } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
                $this->servisPjeseID->setQueryStringValue($keyValue);
                $this->RecKey["servisPjeseID"] = $this->servisPjeseID->QueryStringValue;
            } elseif (!$loadCurrentRecord) {
                $returnUrl = "ServisPjesetList"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                        ...
                        if (!$res) { // Load record based on key
                            ...
                            $returnUrl = "ServisPjesetList"; // No matching record, return to list
                        }
                    break;
            }
        } else {
            $returnUrl = "ServisPjesetList"; // Not page request, return to list
        }
        if ($returnUrl != "") {
