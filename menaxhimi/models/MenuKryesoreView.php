 ...
 * Page class
 */
class MenuKryesoreView extends MenuKryesore
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "MenuKryesoreView";
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("menukID") ?? Route("menukID")) !== null) {
                ...
                $this->RecKey["menukID"] = $this->menukID->QueryStringValue;
            } elseif (Post("menukID") !== null) {
                $this->menukID->setFormValue(Post("menukID"));
                $this->RecKey["menukID"] = $this->menukID->FormValue;
            } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
                $this->menukID->setQueryStringValue($keyValue);
                $this->RecKey["menukID"] = $this->menukID->QueryStringValue;
            } elseif (!$loadCurrentRecord) {
                $returnUrl = "MenuKryesoreList"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                        ...
                        if (!$res) { // Load record based on key
                            ...
                            $returnUrl = "MenuKryesoreList"; // No matching record, return to list
                        }
                    break;
            }
        } else {
            $returnUrl = "MenuKryesoreList"; // Not page request, return to list
        }
        if ($returnUrl != "") {
