...
class BlogEdit extends Blog
{
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("blogID") ?? Key(0) ?? Route(2)) !== null) {
                $this->blogID->setQueryStringValue($keyValue);
                $this->blogID->setOldValue($this->blogID->QueryStringValue);
            } elseif (Post("blogID") !== null) {
                $this->blogID->setFormValue(Post("blogID"));
                $this->blogID->setOldValue($this->blogID->FormValue);
            } else {
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->blogID->IsDetailKey) {
            $this->blogID->setFormValue($val);
        }
        ...
        // Check field name 'blogModifikuar' first before field var 'x_blogModifikuar'
        $val = $CurrentForm->hasValue("blogModifikuar") ? $CurrentForm->getValue("blogModifikuar") : $CurrentForm->getValue("x_blogModifikuar");
        if (!$this->blogModifikuar->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->blogModifikuar->Visible = false; // Disable update for API request
            } else {
                $this->blogModifikuar->setFormValue($val);
            }
            $this->blogModifikuar->CurrentValue = UnFormatDateTime($this->blogModifikuar->CurrentValue, $this->blogModifikuar->formatPattern());
        }
		$this->blogFoto->OldUploadPath = '../ngarkime/blog/';
		$this->blogFoto->UploadPath = $this->blogFoto->OldUploadPath;
    ...
    public function restoreFormValues()
    {
        ...
        $this->blogModifikuar->CurrentValue = $this->blogModifikuar->FormValue;
        $this->blogModifikuar->CurrentValue = UnFormatDateTime($this->blogModifikuar->CurrentValue, $this->blogModifikuar->formatPattern());
    }
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
        if ($this->blogID->Required) {
            if (!$this->blogID->IsDetailKey && EmptyValue($this->blogID->FormValue)) {
                $this->blogID->addErrorMessage(str_replace("%s", $this->blogID->caption(), $this->blogID->RequiredErrorMessage));
            }
        }
        ...
        if ($this->blogModifikuar->Required) {
            if (!$this->blogModifikuar->IsDetailKey && EmptyValue($this->blogModifikuar->FormValue)) {
                $this->blogModifikuar->addErrorMessage(str_replace("%s", $this->blogModifikuar->caption(), $this->blogModifikuar->RequiredErrorMessage));
            }
        }
    ...
    protected function editRow()
    {
        ...
        // blogGjuha
        $this->blogGjuha->setDbValueDef($rsnew, $this->blogGjuha->CurrentValue, "", $this->blogGjuha->ReadOnly);

        // blogKategoria
        $this->blogKategoria->setDbValueDef($rsnew, $this->blogKategoria->CurrentValue, 0, $this->blogKategoria->ReadOnly);

        // blogTitulli
        $this->blogTitulli->setDbValueDef($rsnew, $this->blogTitulli->CurrentValue, "", $this->blogTitulli->ReadOnly);

        // blogTxt
        $this->blogTxt->setDbValueDef($rsnew, $this->blogTxt->CurrentValue, "", $this->blogTxt->ReadOnly);

        // blogFoto
        if ($this->blogFoto->Visible && !$this->blogFoto->ReadOnly && !$this->blogFoto->Upload->KeepFile) {
            $this->blogFoto->Upload->DbValue = $rsold['blogFoto']; // Get original value
        ...
        // blogVideo
        $this->blogVideo->setDbValueDef($rsnew, $this->blogVideo->CurrentValue, null, $this->blogVideo->ReadOnly);

        // blogDtPublik
        $this->blogDtPublik->setDbValueDef($rsnew, UnFormatDateTime($this->blogDtPublik->CurrentValue, $this->blogDtPublik->formatPattern()), CurrentDate(), $this->blogDtPublik->ReadOnly);
        ...
        // blogModifikuar
        $this->blogModifikuar->CurrentValue = CurrentDateTime();
        $this->blogModifikuar->setDbValueDef($rsnew, $this->blogModifikuar->CurrentValue, null);
        ...
        if ($this->blogFoto->Visible && !$this->blogFoto->Upload->KeepFile) {
            ...
            if (!EmptyValue($this->blogFoto->Upload->FileName)) {
                ...
                $this->blogFoto->setDbValueDef($rsnew, $this->blogFoto->Upload->FileName, "", $this->blogFoto->ReadOnly);
            }
        }
