 ...
 * Page class
 */
class PerdoruesitView extends Perdoruesit
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PerdoruesitView";
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("perdID") ?? Route("perdID")) !== null) {
                ...
                $this->RecKey["perdID"] = $this->perdID->QueryStringValue;
            } elseif (Post("perdID") !== null) {
                $this->perdID->setFormValue(Post("perdID"));
                $this->RecKey["perdID"] = $this->perdID->FormValue;
            } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
                $this->perdID->setQueryStringValue($keyValue);
                $this->RecKey["perdID"] = $this->perdID->QueryStringValue;
            } elseif (!$loadCurrentRecord) {
                $returnUrl = "PerdoruesitList"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                        ...
                        if (!$res) { // Load record based on key
                            ...
                            $returnUrl = "PerdoruesitList"; // No matching record, return to list
                        }
                    break;
            }
        } else {
            $returnUrl = "PerdoruesitList"; // Not page request, return to list
        }
        if ($returnUrl != "") {
