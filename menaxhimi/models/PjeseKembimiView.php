 ...
 * Page class
 */
class PjeseKembimiView extends PjeseKembimi
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PjeseKembimiView";
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("pjeseID") ?? Route("pjeseID")) !== null) {
                ...
                $this->RecKey["pjeseID"] = $this->pjeseID->QueryStringValue;
            } elseif (Post("pjeseID") !== null) {
                $this->pjeseID->setFormValue(Post("pjeseID"));
                $this->RecKey["pjeseID"] = $this->pjeseID->FormValue;
            } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
                $this->pjeseID->setQueryStringValue($keyValue);
                $this->RecKey["pjeseID"] = $this->pjeseID->QueryStringValue;
            } elseif (!$loadCurrentRecord) {
                $returnUrl = "PjeseKembimiList"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                        ...
                        if (!$res) { // Load record based on key
                            ...
                            $returnUrl = "PjeseKembimiList"; // No matching record, return to list
                        }
                    break;
            }
        } else {
            $returnUrl = "PjeseKembimiList"; // Not page request, return to list
        }
        if ($returnUrl != "") {
