 ...
 * Page class
 */
class ServisPjesetList extends ServisPjeset
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ServisPjesetList";
    ...
    public $FormName = "fservis_pjesetlist";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "ServisPjesetAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "ServisPjesetDelete";
        $this->MultiUpdateUrl = "ServisPjesetUpdate";
    ...
    public function run()
    {
        ...
        // Load master record
        if ($this->CurrentMode != "add" && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "servis") {
            $masterTbl = Container("servis");
            $rsmaster = $masterTbl->loadRs($this->DbMasterFilter)->fetchAssociative();
            $this->MasterRecordExists = $rsmaster !== false;
            if (!$this->MasterRecordExists) {
                $this->setFailureMessage($Language->phrase("NoRecord")); // Set no record found
                $this->terminate("ServisList"); // Return to master page
                return;
            } else {
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server" && isset($UserProfile)) {
            $savedFilterList = $UserProfile->getSearchFilters(CurrentUserName(), "fservis_pjesetsrch");
        }
        $filterList = Concat($filterList, $this->servisPjeseID->AdvancedSearch->toJson(), ","); // Field servisPjeseID
        $filterList = Concat($filterList, $this->servisPjeseServisID->AdvancedSearch->toJson(), ","); // Field servisPjeseServisID
        $filterList = Concat($filterList, $this->servisPjesePjesa->AdvancedSearch->toJson(), ","); // Field servisPjesePjesa
        $filterList = Concat($filterList, $this->servisPjeseSasia->AdvancedSearch->toJson(), ","); // Field servisPjeseSasia
        $filterList = Concat($filterList, $this->servisPjeseCmimi->AdvancedSearch->toJson(), ","); // Field servisPjeseCmimi
        $filterList = Concat($filterList, $this->servisPjeseShenim->AdvancedSearch->toJson(), ","); // Field servisPjeseShenim
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fservis_pjesetsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field servisPjeseID
        $this->servisPjeseID->AdvancedSearch->SearchValue = @$filter["x_servisPjeseID"];
        $this->servisPjeseID->AdvancedSearch->SearchOperator = @$filter["z_servisPjeseID"];
        $this->servisPjeseID->AdvancedSearch->SearchCondition = @$filter["v_servisPjeseID"];
        $this->servisPjeseID->AdvancedSearch->SearchValue2 = @$filter["y_servisPjeseID"];
        $this->servisPjeseID->AdvancedSearch->SearchOperator2 = @$filter["w_servisPjeseID"];
        $this->servisPjeseID->AdvancedSearch->save();

        // Field servisPjeseServisID
        $this->servisPjeseServisID->AdvancedSearch->SearchValue = @$filter["x_servisPjeseServisID"];
        $this->servisPjeseServisID->AdvancedSearch->SearchOperator = @$filter["z_servisPjeseServisID"];
        $this->servisPjeseServisID->AdvancedSearch->SearchCondition = @$filter["v_servisPjeseServisID"];
        $this->servisPjeseServisID->AdvancedSearch->SearchValue2 = @$filter["y_servisPjeseServisID"];
        $this->servisPjeseServisID->AdvancedSearch->SearchOperator2 = @$filter["w_servisPjeseServisID"];
        $this->servisPjeseServisID->AdvancedSearch->save();

        // Field servisPjesePjesa
        $this->servisPjesePjesa->AdvancedSearch->SearchValue = @$filter["x_servisPjesePjesa"];
        $this->servisPjesePjesa->AdvancedSearch->SearchOperator = @$filter["z_servisPjesePjesa"];
        $this->servisPjesePjesa->AdvancedSearch->SearchCondition = @$filter["v_servisPjesePjesa"];
        $this->servisPjesePjesa->AdvancedSearch->SearchValue2 = @$filter["y_servisPjesePjesa"];
        $this->servisPjesePjesa->AdvancedSearch->SearchOperator2 = @$filter["w_servisPjesePjesa"];
        $this->servisPjesePjesa->AdvancedSearch->save();

        // Field servisPjeseSasia
        $this->servisPjeseSasia->AdvancedSearch->SearchValue = @$filter["x_servisPjeseSasia"];
        $this->servisPjeseSasia->AdvancedSearch->SearchOperator = @$filter["z_servisPjeseSasia"];
        $this->servisPjeseSasia->AdvancedSearch->SearchCondition = @$filter["v_servisPjeseSasia"];
        $this->servisPjeseSasia->AdvancedSearch->SearchValue2 = @$filter["y_servisPjeseSasia"];
        $this->servisPjeseSasia->AdvancedSearch->SearchOperator2 = @$filter["w_servisPjeseSasia"];
        $this->servisPjeseSasia->AdvancedSearch->save();

        // Field servisPjeseCmimi
        $this->servisPjeseCmimi->AdvancedSearch->SearchValue = @$filter["x_servisPjeseCmimi"];
        $this->servisPjeseCmimi->AdvancedSearch->SearchOperator = @$filter["z_servisPjeseCmimi"];
        $this->servisPjeseCmimi->AdvancedSearch->SearchCondition = @$filter["v_servisPjeseCmimi"];
        $this->servisPjeseCmimi->AdvancedSearch->SearchValue2 = @$filter["y_servisPjeseCmimi"];
        $this->servisPjeseCmimi->AdvancedSearch->SearchOperator2 = @$filter["w_servisPjeseCmimi"];
        $this->servisPjeseCmimi->AdvancedSearch->save();

        // Field servisPjeseShenim
        $this->servisPjeseShenim->AdvancedSearch->SearchValue = @$filter["x_servisPjeseShenim"];
        $this->servisPjeseShenim->AdvancedSearch->SearchOperator = @$filter["z_servisPjeseShenim"];
        $this->servisPjeseShenim->AdvancedSearch->SearchCondition = @$filter["v_servisPjeseShenim"];
        $this->servisPjeseShenim->AdvancedSearch->SearchValue2 = @$filter["y_servisPjeseShenim"];
        $this->servisPjeseShenim->AdvancedSearch->SearchOperator2 = @$filter["w_servisPjeseShenim"];
        $this->servisPjeseShenim->AdvancedSearch->save();
    ...
    protected function basicSearchWhere($default = false)
    {
        ...
        // Fields to search
        $searchFlds = [];
        $searchFlds[] = &$this->servisPjeseShenim;
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->servisPjeseID, $ctrl); // servisPjeseID
            $this->updateSort($this->servisPjeseServisID, $ctrl); // servisPjeseServisID
            $this->updateSort($this->servisPjesePjesa, $ctrl); // servisPjesePjesa
            $this->updateSort($this->servisPjeseSasia, $ctrl); // servisPjeseSasia
            $this->updateSort($this->servisPjeseCmimi, $ctrl); // servisPjeseCmimi
            $this->updateSort($this->servisPjeseShenim, $ctrl); // servisPjeseShenim
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->servisPjeseID->setSort("");
                $this->servisPjeseServisID->setSort("");
                $this->servisPjesePjesa->setSort("");
                $this->servisPjeseSasia->setSort("");
                $this->servisPjeseCmimi->setSort("");
                $this->servisPjeseShenim->setSort("");
            }
    ...
    public function renderListOptions()
    {
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fservis_pjesetlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fservis_pjesetlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->servisPjeseID->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("servisPjeseID", $this->createColumnOption("servisPjeseID"));
            $option->add("servisPjeseServisID", $this->createColumnOption("servisPjeseServisID"));
            $option->add("servisPjesePjesa", $this->createColumnOption("servisPjesePjesa"));
            $option->add("servisPjeseSasia", $this->createColumnOption("servisPjeseSasia"));
            $option->add("servisPjeseCmimi", $this->createColumnOption("servisPjeseCmimi"));
            $option->add("servisPjeseShenim", $this->createColumnOption("servisPjeseShenim"));
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fservis_pjesetsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fservis_pjesetsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fservis_pjesetlist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->servisPjeseShenim->TooltipValue = "";
                    ...
                    $this->servisPjeseCmimi->Total = 0; // Initialize total
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="fservis_pjesetlist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fservis_pjesetsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    */
    public function exportData($return = false)
    {
        ...
        if (Config("EXPORT_MASTER_RECORD") && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "servis") {
            $servis = Container("servis");
            $rsmaster = $servis->loadRs($this->DbMasterFilter); // Load master record
            if ($rsmaster) {
                $exportStyle = $doc->Style;
                $doc->setStyle("v"); // Change to vertical
                if (!$this->isExport("csv") || Config("EXPORT_MASTER_RECORD_FOR_CSV")) {
                    $doc->Table = $servis;
                    $servis->exportDocument($doc, new Recordset($rsmaster));
                    $doc->exportEmptyRow();
                    $doc->Table = &$this;
                }
